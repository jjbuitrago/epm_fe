...
def paint_map_antioquia(min_date, max_date, priority_range, town_value):
    ...
    if len(priority_range) > 0:
        # comma separated values for DB query
        pri_range = ','.join(["'"+str(x)+"'" for x in priority_range])
    ...
    grp['town_join'] = grp['town'].str.normalize('NFKD').str.encode(
        'ascii', errors='ignore').str.decode('utf-8').str.upper()
    # match to obtain keys to the geojson
    grp = grp.merge(shape_MPIO[['MPIO_CNMBR', 'MPIO_CCNCT']], how='inner',
                    left_on='town_join', right_on='MPIO_CNMBR').drop(columns=['MPIO_CNMBR'])
...
def paint_prll_categ(df):
    ...
    fig = px.parallel_categories(df, dimensions=[
                                 "priority", 'servicetype', 'servicegroup'], color='priority', color_continuous_scale=px.colors.qualitative.Plotly)
...
def paint_time_boxes(df):
    ...
    # creating boxplots
    duration = px.box(df, y='duratiomin', x='priority',
                      color='priority', log_y=True)

    travel = px.box(df, y='travel_to_onsite', x='priority',
                    color='priority', log_y=True)

    open_to_completion = px.box(
        df, y='open_to_completion', x='priority', color='priority', log_y=True)
...
def paint_descriptive_graphs(min_date, max_date, priority_range, town_value):
    ...
    duration_box, travel_box, open2completion_box = paint_time_boxes(
        epm_filtered)
...
for i in np.arange(0, len(antioquia_mpio_json['features'])):
    ...
    rows.append(pd.DataFrame.from_dict(
        antioquia_mpio_json['features'][i]['properties'], orient='index').T[['MPIO_CNMBR', 'MPIO_CCNCT']])
...
dict_priority = [{'label': y, 'value': x}
                 for x, y in zip(priority_range, priority_range)]
...
Map_Fig = paint_map_antioquia(
    min_date, max_date, priority_range, dict_towns[-1]['value'])
...
prll_categ, duration_box, travel_box, open2completion_box = paint_descriptive_graphs(
    min_date, max_date, priority_range, dict_towns[-1]['value'])
...
##############################
map = html.Div([
    dcc.Graph(figure=Map_Fig, id='ant_map'), html.Div([date_picker]), html.Div([priority_dropdown]), html.Div([town_dropdown]), dcc.Graph(figure=prll_categ, id='prll_categ'), html.Div([
        dcc.Graph(figure=duration_box, id='duration_box'), dcc.Graph(figure=travel_box,
                                                                     id='travel_box'), dcc.Graph(figure=open2completion_box, id='open_to_completion_box')
    ])
],
    className="ds4a-body")
...
def update_figs_date(start_date, end_date, value_p, value_t):
    ...
    Map_Fig = paint_map_antioquia(
        start_date, end_date, sel_priorities, town_value)
